        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-4 max-w-5xl text-sm leading-7 text-white">
              ...
              requests, governance review, document-control actions, certified-copy
              support, treasury support, and restricted HCA review preparation.
            </p>
          </div>
          ...
          <div className="rounded-lg border border-[#ff8a00] bg-white/5 px-10 py-7 text-center">
            ...
            <p className="mt-1 text-xs font-black">Intake Routing Layer</p>
          </div>
        </div>
      </section>
      ...
      <section className="mt-5 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="grid gap-4 xl:grid-cols-[1fr_220px_260px_140px]">
          <div>
            ...
            <div className="mt-2 flex h-12 items-center gap-3 rounded-lg border border-slate-300 px-4">
          ...
          <div>
            ...
            <select
              ...
              className="mt-2 h-12 w-full rounded-lg border border-slate-300 bg-white px-4 text-sm font-black outline-none"
            >
          ...
          <button
            ...
            className="mt-auto h-12 rounded-lg border border-slate-300 bg-white px-4 text-sm font-black shadow-sm transition hover:border-[#ff8a00]"
          >
            Clear Filters
          </button>
        </div>
        ...
        <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
          <div className="flex flex-col gap-4 border-b border-slate-200 p-6 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <Link
                ...
                className="inline-flex items-center gap-2 rounded-lg bg-[#050816] px-5 py-4 text-sm font-black text-white transition hover:bg-[#111827]"
              >
                <Plus size={16} />
          ...
          <div className="overflow-x-auto">
            <table className="min-w-[980px] w-full text-left">
              <thead className="bg-slate-50 text-xs font-black uppercase tracking-[0.35em] text-slate-500">
                <tr>
              ...
              <tbody>
                {filteredRequests.map((request) => {
                  ...
                  return (
                    ...
                    >
                      <td className="px-6 py-5 align-top text-sm font-black">
                        {request.id}
                      </td>
                      ...
                      <td className="px-6 py-5 align-top">
                        <Link
                          ...
                          onClick={(event) => event.stopPropagation()}
                          className="inline-flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-black transition hover:border-[#ff8a00] hover:bg-amber-50"
                        >
                          Open
                          <ArrowRight size={15} className="text-[#ff8a00]" />
                        </Link>
                      </td>
                    </tr>
                  );
  ...
  return (
    <aside className="space-y-5">
      ...
      <InfoNote
        ...
        text="Service requests serve as universal intake. Every request should have an assigned queue, controlled visibility, and a governed next action."
      />
      <InfoNote
        title="Backend Readiness"
        text="Future work should connect this desk to authenticated submissions, role-based assignment, department queues, comments, attachments, approvals, and permanent records."
      />
    </aside>
  );
}
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm font-black">{request.title}</p>
          </div>
          <span
            className={`rounded-md px-3 py-2 text-xs font-black ${getStatusPillClass(
              request.status,
        ...
        <div className="mt-5 space-y-3">
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-lg border border-slate-300 bg-white px-4 py-3 text-sm font-black transition hover:border-[#ff8a00]"
          >
          ...
          <Link
            href={`/service-requests/${request.id}`}
            className="flex w-full items-center justify-center gap-2 rounded-lg bg-[#050816] px-4 py-3 text-sm font-black text-white transition hover:bg-[#111827]"
          >
...
function InfoNote({ title, text }: { title: string; text: string }) {
  return (
    <section className="rounded-xl border border-dashed border-slate-300 bg-white p-6 shadow-sm">
      <h3 className="text-xl font-black uppercase tracking-[0.25em]">{title}</h3>
